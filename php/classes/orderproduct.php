...
//		echo 'end of orderProduct delete<br>';
	...
	/**
	 * gets all order products by product id
	 *
	 * @param resource $mysqli pointer to mySQL connection, by reference
	 ...
	 * @throws mysqli_sql_exception when mySQL related errors occur
	 **/
	public static function getAllOrderProductsByOrderId(&$mysqli, $orderId) {
		// handle degenerate cases
		if(gettype($mysqli) !== "object" || get_class($mysqli) !== "mysqli") {
			throw(new mysqli_sql_exception("input is not a mysqli object"));
		}
		...
		// create query template
		$query	 = "SELECT orderId, productId, locationId, productQuantity FROM orderProduct WHERE orderId = ?";
		$statement = $mysqli->prepare($query);
		if($statement === false) {
			throw(new mysqli_sql_exception("unable to prepare statement"));
		}
